	...
	private static enum AlgorithmVER01 implements CombinationAlgorithm {
		...
		public BigInteger getCombinationCount(int numberOfElements, int numberToPick) {
			...
			if (numberToPick < 0 || numberToPick > numberOfElements)
				throw new IllegalArgumentException("Invalid number to pick : " + numberToPick);

			if (numberToPick == 0 || numberToPick == numberOfElements)
				return ONE;
			else
				return Calculator.factorial(numberOfElements).divide(Calculator.factorial(numberToPick)
						.multiply(Calculator.factorial(numberOfElements - numberToPick)));
		}
